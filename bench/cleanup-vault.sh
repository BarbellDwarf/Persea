...
# Usage: ./cleanup-vault.sh [api_key] [persea_url]
set -e

API_KEY=${1:-""}
PERSEA_URL=${2:-"https://localhost:8089"}

if [ -z "$API_KEY" ]; then
    echo "Usage: $0 <api_key> [persea_url]"
    exit 1
fi
...
FOLDERS=$($CURL "$PERSEA_URL/api/addressbook" | python3 -c "
import sys, json
...
for folder in $FOLDERS; do
    ...
    $CURL -X DELETE "$PERSEA_URL/api/addressbook/folders/$SCOPE/$NAME" -o /dev/null 2>/dev/null
