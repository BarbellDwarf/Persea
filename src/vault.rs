...
/// Abstraction over the Vault storage backend.
///
/// The API layer talks to this trait (via [`crate::api::VaultBackends`])
/// instead of the concrete [`VaultClient`], so handler tests can run against
/// an in-memory fake (`crate::testing::MockVault`) without a live
/// HashiVault/OpenBao server.
///
/// The surface is exactly the methods the API layer and the drive module
/// call on `VaultClient`. Keep it minimal — adding a method here is a
/// breaking change for implementors.
#[async_trait::async_trait]
pub trait VaultBackend: Send + Sync {
    /// List top-level folders for a single scope (`"shared"` or `"instance"`).
    async fn list_folders_in_scope(&self, scope: &str) -> Result<Vec<FolderInfo>, VaultError>;
    /// List immediate subfolders at a folder path.
    ...
    /// List entry names in a folder (excludes `.config`).
    async fn list_entries(&self, scope: &str, folder: &str) -> Result<Vec<String>, VaultError>;
    /// Get a full entry (with credentials).
    async fn get_entry(
        &self,
        scope: &str,
    ...
    /// Delete an entry (all versions).
    async fn delete_entry(&self, scope: &str, folder: &str, entry: &str) -> Result<(), VaultError>;
    /// Get a folder's `.config`.
    ...
    /// Delete a folder and its whole subtree; returns (subfolder_count, entry_count).
    async fn delete_folder(&self, scope: &str, folder: &str) -> Result<(usize, usize), VaultError>;
    /// Resolve whether `user_groups` grants access to `folder` under `scope`.
    ...
    /// Read a user's stored credential variables (full map).
    async fn get_user_credentials(
        &self,
        email: &str,
    ) -> Result<HashMap<String, String>, VaultError>;
    /// Write a user's credential variables (full replace).
    async fn put_user_credentials(
        &self,
        email: &str,
        creds: &HashMap<String, String>,
    ) -> Result<(), VaultError>;
    /// Read a single field from an arbitrary KV v2 path (e.g. the LUKS key).
    async fn read_kv_field(&self, kv_path: &str, field: &str) -> Result<String, VaultError>;
}

#[async_trait::async_trait]
impl VaultBackend for VaultClient {
    async fn list_folders_in_scope(&self, scope: &str) -> Result<Vec<FolderInfo>, VaultError> {
        self.list_folders_in_scope(scope).await
    }
    ...
    async fn list_subfolders(
        ...
        self.list_subfolders(scope, parent).await
    }

    async fn list_entries(&self, scope: &str, folder: &str) -> Result<Vec<String>, VaultError> {
        self.list_entries(scope, folder).await
    }
    ...
    async fn get_entry(
        ...
        self.get_entry(scope, folder, entry).await
    }
    ...
    async fn put_entry(
        ...
        self.put_entry(scope, folder, entry, data).await
    }

    async fn delete_entry(&self, scope: &str, folder: &str, entry: &str) -> Result<(), VaultError> {
        self.delete_entry(scope, folder, entry).await
    }
    ...
    async fn get_folder_config(
        ...
        self.get_folder_config(scope, folder).await
    }
    ...
    async fn put_folder_config(
        ...
        self.put_folder_config(scope, folder, config).await
    }

    async fn delete_folder(&self, scope: &str, folder: &str) -> Result<(usize, usize), VaultError> {
        self.delete_folder(scope, folder).await
    }
    ...
    async fn resolve_folder_access(
        ...
        self.resolve_folder_access(scope, folder, user_groups).await
    }
    ...
    async fn get_user_credentials(
        ...
        self.get_user_credentials(email).await
    }
    ...
    async fn put_user_credentials(
        ...
        self.put_user_credentials(email, creds).await
    }

    async fn read_kv_field(&self, kv_path: &str, field: &str) -> Result<String, VaultError> {
        self.read_kv_field(kv_path, field).await
    }
}
