...
/// Extract a named top-level section (header + body) from a config file,
/// stopping at the next `[` header. Trailing blank lines are dropped so the
/// extracted section ends after its last setting. Returns None when the
/// section is absent.
fn extract_section(text: &str, name: &str) -> Option<String> {
    let header = format!("[{name}]");
    let mut lines = text.lines();
    // starts_with rather than equality so a trailing comment on the header
    // line ("[tls] # production", valid TOML) still matches; safe because a
    // header always ends in ']' and cannot prefix-match a longer name.
    lines.position(|l| l.trim_start().starts_with(&header))?;
...
/// Read the config file from disk and extract the named section, or None
/// when either the file or the section is missing.
fn read_section(config_path: &str, name: &str) -> Option<String> {
...
///
/// `[tls]` is preserved the same way (see `tls_section_for`); both sections
/// are extracted with `extract_section`.
fn storage_section_for(config_path: &str) -> String {
    match read_section(config_path, "storage") {
...
/// The `[tls]` section to write into the generated config: the existing
/// section verbatim, so an HTTPS deployment (cert paths, guacd TLS trust,
/// `secure_cookies`) survives the wizard's rewrite instead of silently
/// downgrading to plain HTTP at the next restart. Empty when the config has
/// no `[tls]` section: HTTP-only installs stay as the wizard made them.
fn tls_section_for(config_path: &str) -> String {
    read_section(config_path, "tls").unwrap_or_default()
}
...
"#
    ...
    // An existing [tls] section is preserved verbatim too (see
    // tls_section_for): dropping it would downgrade an HTTPS deployment to
    // plain HTTP at the next restart.
    let storage_section = storage_section_for(&config_path);
    let tls_section = tls_section_for(&config_path);
...
mod tests {
    ...
    /// Write a throwaway config file for the section-extraction tests and
    /// return its path (the wizard reads sections back from disk).
    fn write_config(name: &str, contents: &str) -> std::path::PathBuf {
        let path = std::env::temp_dir().join(format!(
            "persea-setup-test-{name}-{}.toml",
            std::process::id()
        ));
    ...
    #[test]
    fn extract_section_preserves_tls_verbatim() {
        let config = r#"listen_addr = "0.0.0.0:8089"

[tls]
cert_path = "/etc/persea/cert.pem"
key_path = "/etc/persea/key.pem"
...
secure_cookies = false
"#;
        let tls = extract_section(config, "tls").unwrap();
        assert_eq!(tls, expected);
        // The section stops at the next header: storage content stays out.
        assert!(!tls.contains("encryption_key"));
    }

    #[test]
    fn extract_section_absent_returns_none() {
        let config = "listen_addr = \"0.0.0.0:8089\"\n";
        assert_eq!(extract_section(config, "tls"), None);
        assert_eq!(extract_section(config, "storage"), None);
    }

    #[test]
    fn tls_section_for_preserves_existing_tls_section() {
...
"#,
        ...
        let tls = tls_section_for(path.to_str().unwrap());
        assert_eq!(
            tls,
            r#"[tls]
cert_path = "/etc/persea/cert.pem"
...
"#
    ...
    #[test]
    fn tls_section_for_empty_without_tls_section() {
        // HTTP-only install: the wizard must emit nothing for [tls].
        let path = write_config("tls-absent", "listen_addr = \"127.0.0.1:8089\"\n");
        assert_eq!(tls_section_for(path.to_str().unwrap()), "");
        let _ = std::fs::remove_file(&path);
    }

    #[test]
    fn storage_section_for_preserves_existing_key() {
        let path = write_config(
            "storage-key",
            r#"[tls]
cert_path = "/etc/persea/cert.pem"
...
"#,
        ...
        assert_eq!(
            storage_section_for(path.to_str().unwrap()),
            r#"[storage]
encryption_key = "aabbccdd"
"#
        );
    ...
    #[test]
    fn storage_section_for_adds_key_when_section_lacks_one() {
        let path = write_config(
            "storage-no-key",
            r#"[storage]
backend = "db"
"#,
        ...
        let section = storage_section_for(path.to_str().unwrap());
        assert!(section.starts_with("[storage]\nbackend = \"db\"\n"));
        let key = section
            .lines()
            .find(|l| l.starts_with("encryption_key"))
            .unwrap()
    ...
    #[test]
    fn storage_section_for_generates_fresh_section_when_absent() {
        let path = write_config("storage-absent", "listen_addr = \"127.0.0.1:8089\"\n");
        let section = storage_section_for(path.to_str().unwrap());
        assert!(section.starts_with("[storage]\nencryption_key = \""));
        let key = section
            .lines()
