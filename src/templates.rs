...
// Build-time rendered documentation pages: `(slug, title, html, headings)`
// tuples generated by build.rs from `docs/*.md` (see `DOC_FILES` there).
// `headings` is the list of h2/h3 `(slug, text)` pairs that drive the
// two-level docs nav. The same generated file is included by
// `src/api/admin.rs` for the admin JSON API; this copy feeds the public
// docs page template.
include!(concat!(env!("OUT_DIR"), "/docs-rendered.rs"));

/// One heading within a rendered documentation page: the GitHub-style slug
/// (the element id assigned to the heading in `DocSection::html`) plus the
/// plain heading text. Feeds the subsection level of the docs nav.
#[derive(Serialize)]
pub struct DocHeading {
    /// URL fragment / element id of the heading, e.g. "tls-section".
    pub slug: String,
    /// Plain heading text as rendered, e.g. "[tls] section".
    pub text: String,
}

/// One rendered documentation page, as exposed to the docs template.
#[derive(Serialize)]
pub struct DocSection {
    /// URL fragment / element id for the section, e.g. "configuration".
    pub slug: String,
    /// Page title taken from the first `#` heading in the markdown.
    pub title: String,
    /// Build-time rendered HTML of the markdown page.
    pub html: String,
    /// h2/h3 (slug, text) pairs in document order for the two-level nav;
    /// the slugs are the ids assigned to the headings in `html`.
...
pub struct DocsTemplate {
    ...
    /// CSP nonce that inline scripts in the rendered page must carry.
    pub csp_nonce: String,
    /// Rendered documentation pages, in build order, for the section nav
    /// and the page body.
