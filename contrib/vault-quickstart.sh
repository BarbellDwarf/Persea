#!/usr/bin/env bash
#
# vault-quickstart.sh - Provision Vault or OpenBao for persea connections.
#
...
#              and prints the persea config snippet.
#
...
#   ./vault-quickstart.sh
#
#   # local single-box persea install:
...
#   --base-path NAME     Path under the mount for persea data (default: persea).
#   --policy NAME        persea policy name (default: persea).
#   --role NAME          persea AppRole name (default: persea).
...
set_flavor() {
    case "$CLI" in
        vault)
            ...
            LOCAL_UNSEAL_HELPER="/usr/local/sbin/persea-vault-unseal"
            ;;
        bao)
            ...
            LOCAL_UNSEAL_HELPER="/usr/local/sbin/persea-openbao-unseal"
            ;;
...
start_dev_vault() {
    local dev_log="/tmp/persea-vault-quickstart.log"
...
# /etc/vault.d/unseal-key (0400 root:root). On a single-host persea box
...
# Generated by persea contrib/vault-quickstart.sh - local single-box mode
...
[Unit]
Description=${LOCAL_FLAVOR} for persea
...
# Added by persea contrib/vault-quickstart.sh.
[Service]
ExecStartPost=+${LOCAL_UNSEAL_HELPER}
EOF
    fi
...
this ${LOCAL_FLAVOR} holds. On a single-host persea install where root
...
EOF
    ...
    # Loud warning. The script keeps going so the persea config snippet
...
# persea connection entries (KV v2 data)
path "${MOUNT}/data/${BASE_PATH}/*" {
    capabilities = ["create", "read", "update", "delete"]
}
# persea folder/entry listing and permanent deletion (KV v2 metadata).
...
============================================================
  persea vault provisioning complete
============================================================

Add to /opt/persea/config.toml (or your dev config):
...
Add to the persea systemd env file (default /opt/persea/env)
or export in your shell:

    VAULT_SECRET_ID=${SECRET_ID}

Restart persea and look for:
