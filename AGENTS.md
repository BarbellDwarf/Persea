...
- guacd is built from the maintained fork `persea-grove/persea-guacamole-server`, branch `persea-1.6.1-freerdp3` (apache/guacamole-server pinned at `de97609` with the former `patches/` quilt applied as one commit per patch). See `patches/README.md`.
