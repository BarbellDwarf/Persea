...
- **Cause: the SSO button is missing, or discovery reports an "empty JWKS".** The identity provider publishes no signing key, usually because no signing certificate is selected on its OIDC provider (authentik serves an empty JWKS when the provider's **Signing Key** field is empty). Fix: select a certificate-key pair as the signing key on the provider in your IdP (prefer RSA), confirm the provider's `jwks_uri` now returns a JSON document containing a non-empty `keys` array, then restart persea. The SSO registry is built once at startup, so a provider that failed discovery stays absent from the login page until a restart.
