...
It enables AVC 4:4:4, 60 FPS, desktop composition, and GPU encoding. Note that Windows only sends H.264 when a GPU (physical or virtual) is present; without one it falls back to RemoteFX/Planar, which guacd re-encodes as JPEG/WebP — still good quality, just not as low-latency.
