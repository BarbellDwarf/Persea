...
### Bind mounts and user mapping

Inside the container persea runs as user `persea`, uid/gid 996 after startup. Named volumes need nothing from you: the image owns them.

Bind mounts bring their host ownership along, and a directory owned by some other uid would normally be unwritable for uid 996. The entrypoint handles this when the container starts as root (the default): it reconciles ownership of `/opt/persea/data`, `recordings`, `tls`, `certs`, `drives`, `scripts`, and `vdi-homes` to the container user, then drops privileges and continues booting as before.

Be aware of the side effect: with default settings, a normal start **changes the owner of bind-mounted directories (and everything inside them) on the host to 996**. If you want the files to keep their current host owner, tell the container who that is:

```bash
docker run -d -p 8089:8089 \
  -e PUID=1000 -e PGID=1000 \
  -v /srv/persea/data:/opt/persea/data \
  ghcr.io/persea-grove/persea:latest
```

The container user then takes over those uid/gid values, and reconciliation matches the host owner instead of rewriting it. Omitting both variables uses 996.

Some platforms never hand the container root in the first place: `docker run --user …`, Kubernetes with `runAsNonRoot`, or arbitrary-UID platforms such as OpenShift. The entrypoint detects this and skips reconciliation entirely, so mount permissions are your platform's job: set the pod's `fsGroup` to 996, or pre-create the mounted directories owned by the uid you run as. Everything else from [First run](#first-run) still applies unchanged: TLS certificates, config.toml, and the admin key are generated by persea itself once the mounts are writable.
