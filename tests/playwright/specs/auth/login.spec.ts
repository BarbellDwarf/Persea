...
// Regression test: on a fresh instance / brand-new client
...
test.describe('Fresh-instance login routing', () => {
  test('POST /auth/login is reachable with no prior session cookie', async ({ request }) => {
    ...
    // require_auth (the routing concern) from CSRF protection (a separate,
    ...
    // middleware — the exact regression. A redirect (whether to the
