...
async fn set_user_role_updates() {
    ...
    let resp = router.oneshot(admin_put(&key, "/api/users/user@test.com/role", serde_json::json!({"role": "operator"}))).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    assert_eq!(db::get_user_by_email(&db, "user@test.com").unwrap().role, "operator");
}
...
async fn set_role_invalid() {
    ...
    create_user(&db, "user@test.com", "User", "viewer");
    let router = test_router(db);
    let resp = router.oneshot(admin_put(&key, "/api/users/user@test.com/role", serde_json::json!({"role": "superadmin"}))).await.unwrap();
...
async fn set_role_not_found() {
    ...
    let key = create_admin(&db, "admin");
    let router = test_router(db);
    let resp = router.oneshot(admin_put(&key, "/api/users/nobody@test.com/role", serde_json::json!({"role": "operator"}))).await.unwrap();
...
async fn delete_user_works() {
    ...
    create_user(&db, "user@test.com", "User", "viewer");
    let router = test_router(db.clone());
    let resp = router.oneshot(admin_del(&key, "/api/users/user@test.com")).await.unwrap();
...
async fn delete_user_not_found() {
    ...
    let key = create_admin(&db, "admin");
    let router = test_router(db);
    let resp = router.oneshot(admin_del(&key, "/api/users/nobody@test.com")).await.unwrap();
    assert_eq!(resp.status(), StatusCode::NOT_FOUND);
}
...
async fn disable_enable_user() {
    ...
    let r1 = test_router(db.clone()).oneshot(admin_post(&key, "/api/users/user@test.com/disable", serde_json::json!({}))).await.unwrap();
    assert_eq!(r1.status(), StatusCode::OK);
    assert!(db::get_user_by_email(&db, "user@test.com").unwrap().disabled);

    let r2 = test_router(db.clone()).oneshot(admin_post(&key, "/api/users/user@test.com/enable", serde_json::json!({}))).await.unwrap();
...
async fn viewer_cannot_list_users() {
    ...
    let router = test_router(db);
    let resp = router.oneshot(sess_req("GET", "/api/users", &session)).await.unwrap();
...
async fn list_tokens_empty() {
    ...
    let router = test_router(db);
    let resp = router.oneshot(sess_req("GET", "/api/me/tokens", &session)).await.unwrap();
    let json = body_json(resp).await;
    assert!(json.as_array().unwrap().is_empty());
}
...
async fn create_and_list_token() {
    ...
    let router = test_router(db.clone());
    let resp = router.oneshot(sess_post("/api/me/tokens", &session, serde_json::json!({"name": "my-token"}))).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    let json = body_json(resp).await;
...
async fn create_token_empty_name() {
    ...
    let session = setup_session(&db, "user@test.com", "poweruser");
    let router = test_router(db);
    let resp = router.oneshot(sess_post("/api/me/tokens", &session, serde_json::json!({"name": ""}))).await.unwrap();
    assert_eq!(resp.status(), StatusCode::INTERNAL_SERVER_ERROR);
}
...
async fn create_token_duplicate_conflict() {
    let db = test_db();
    let session = setup_session(&db, "user@test.com", "poweruser");
    let r1 = test_router(db.clone()).oneshot(sess_post("/api/me/tokens", &session, serde_json::json!({"name": "dup"}))).await.unwrap();
    assert_eq!(r1.status(), StatusCode::OK);
    let r2 = test_router(db).oneshot(sess_post("/api/me/tokens", &session, serde_json::json!({"name": "dup"}))).await.unwrap();
...
async fn revoke_token() {
    ...
    let r = test_router(db.clone()).oneshot(sess_post("/api/me/tokens", &session, serde_json::json!({"name": "revoke-me"}))).await.unwrap();
    let json = body_json(r).await;
    let token_id = json["id"].as_i64().unwrap();

    let r = test_router(db.clone()).oneshot(sess_req("DELETE", &format!("/api/me/tokens/{}", token_id), &session)).await.unwrap();
...
async fn revoke_nonexistent_token() {
    ...
    let session = setup_session(&db, "user@test.com", "poweruser");
    let router = test_router(db);
    let resp = router.oneshot(sess_req("DELETE", "/api/me/tokens/99999", &session)).await.unwrap();
    assert_eq!(resp.status(), StatusCode::NOT_FOUND);
}
...
async fn viewer_cannot_create_token() {
    ...
    let router = test_router(db);
    let resp = router.oneshot(sess_post("/api/me/tokens", &session, serde_json::json!({"name": "nope"}))).await.unwrap();
    assert_eq!(resp.status(), StatusCode::FORBIDDEN);
}
...
async fn admin_list_tokens_empty() {
    ...
    let key = create_admin(&db, "admin");
    let router = test_router(db);
    let resp = router.oneshot(admin_get(&key, "/api/admin/user-tokens")).await.unwrap();
    let json = body_json(resp).await;
    assert!(json.as_array().unwrap().is_empty());
}
...
async fn admin_create_token_for_user() {
    ...
    create_user(&db, "user@test.com", "User", "viewer");
    let router = test_router(db.clone());
    let resp = router.oneshot(admin_post(&key, "/api/admin/user-tokens", serde_json::json!({"email": "user@test.com", "name": "admin-token"}))).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    let json = body_json(resp).await;
...
async fn admin_create_token_nonexistent_user() {
    ...
    let key = create_admin(&db, "admin");
    let router = test_router(db);
    let resp = router.oneshot(admin_post(&key, "/api/admin/user-tokens", serde_json::json!({"email": "ghost@test.com", "name": "ghost"}))).await.unwrap();
    assert_eq!(resp.status(), StatusCode::NOT_FOUND);
}
...
async fn admin_revoke_token() {
    ...
    let r = test_router(db.clone()).oneshot(admin_post(&key, "/api/admin/user-tokens", serde_json::json!({"email": "user@test.com", "name": "revoke-me"}))).await.unwrap();
    let json = body_json(r).await;
    let token_id = json["id"].as_i64().unwrap();

    let r = test_router(db.clone()).oneshot(admin_del(&key, &format!("/api/admin/user-tokens/{}", token_id))).await.unwrap();
    assert_eq!(r.status(), StatusCode::OK);
...
async fn admin_revoke_nonexistent_token() {
    ...
    let key = create_admin(&db, "admin");
    let router = test_router(db);
    let resp = router.oneshot(admin_del(&key, "/api/admin/user-tokens/99999")).await.unwrap();
    assert_eq!(resp.status(), StatusCode::NOT_FOUND);
}
...
async fn poweruser_cannot_admin_tokens() {
    ...
    let router = test_router(db);
    let resp = router.oneshot(sess_req("GET", "/api/admin/user-tokens", &session)).await.unwrap();
    assert_eq!(resp.status(), StatusCode::FORBIDDEN);
}
