...
proptest! {
    ...
    fn parse_never_panics_on_any_utf8(input in "\\PC*") {
        let _ = rustguac::protocol::Instruction::parse(&input);
    }

    #[test]
    fn parse_never_panics_on_arbitrary_bytes(input in prop::collection::vec(0u8..=255, 0..1000)) {
        let s = String::from_utf8_lossy(&input);
        let _ = rustguac::protocol::Instruction::parse(&s);
    }

    #[test]
    fn encode_then_parse_roundtrip(opcode in "[a-zA-Z]{1,20}", args in prop::collection::vec("[a-zA-Z0-9]{0,50}", 0..10)) {
        let instr = rustguac::protocol::Instruction::new(&opcode, args.clone());
        let encoded = instr.encode();
        let parsed = rustguac::protocol::Instruction::parse(&encoded).unwrap();
    ...
    fn parser_never_panics_on_any_utf8(input in "\\PC*") {
        let mut parser = rustguac::protocol::InstructionParser::new();
